...
function Shops({data,setData,money}:{data:AppData;setData:React.Dispatch<React.SetStateAction<AppData>>;money:(n:number)=>string}) {
  ...
  const list=data.shops.filter(s=>(s.name+" "+s.owner+" "+s.phone+" "+s.area).toLowerCase().includes(search.toLowerCase()));
  ...
  <div className="panel table-panel">{list.length===0?<Empty text="No shops found."/>:<table><thead><tr><th>Shop</th><th>Owner</th><th>Phone</th><th>Balance</th><th>Status</th><th></th></tr></thead><tbody>{list.map(s=>{const b=shopBalance(data,s.id);return <tr key={s.id}><td><b>{s.name}</b><small>{s.area||s.address||"—"}</small></td><td>{s.owner||"—"}</td><td>{s.phone||"—"}</td><td><b>{money(Math.max(0,b))}</b></td><td><span className={`status ${b>0?"pending":"paid"}`}>{b>0?"PENDING":"CLEAR"}</span></td><td><div className="row-actions"><button className="icon-btn" title="Edit" onClick={()=>setEdit(s)}><Pencil size={16}/></button><button className="icon-btn danger" title="Delete" onClick={()=>remove(s.id)}><Trash2 size={16}/></button></div></td></tr>})}</tbody></table>}</div>
  {(add||edit)&&<ShopModal shop={edit} onClose={()=>{setAdd(false);setEdit(null)}} onSave={s=>{setData(d=>({...d,shops:edit?d.shops.map(x=>x.id===s.id?s:x):[...d.shops,s]}));setAdd(false);setEdit(null)}}/>}</>
}

function ShopModal({shop,onClose,onSave}:{shop:Shop|null;onClose:()=>void;onSave:(s:Shop)=>void}) {
  const [f,setF]=useState<Shop>(shop||{id:crypto.randomUUID(),name:"",owner:"",phone:"",address:"",area:"",notes:"",active:true,createdAt:new Date().toISOString()});
  const set=(k:keyof Shop,v:any)=>setF(x=>({...x,[k]:v}));
  return <Modal title={shop?"Edit Shop":"Add Shop"} onClose={onClose}><div className="form-grid"><label>Shop Name *<input value={f.name} onChange={e=>set("name",e.target.value)}/></label><label>Owner Name<input value={f.owner} onChange={e=>set("owner",e.target.value)}/></label><label>Phone<input value={f.phone} onChange={e=>set("phone",e.target.value)}/></label><label>Area / Location<input value={f.area} onChange={e=>set("area",e.target.value)}/></label><label className="wide">Address<textarea value={f.address} onChange={e=>set("address",e.target.value)}/></label><label className="wide">Notes<textarea value={f.notes} onChange={e=>set("notes",e.target.value)}/></label></div><div className="modal-actions"><button className="secondary" onClick={onClose}>Cancel</button><button className="primary" disabled={!f.name.trim()} onClick={()=>onSave(f)}>Save Shop</button></div></Modal>
}
...
function Collection({data,setData,money}:{data:AppData;setData:React.Dispatch<React.SetStateAction<AppData>>;money:(n:number)=>string}) {
  const [date,setDate]=useState(today()),[shopId,setShopId]=useState(""),[amount,setAmount]=useState(""),[method,setMethod]=useState("Cash"),[notes,setNotes]=useState("");
  const due=shopId?Math.max(0,shopBalance(data,shopId,date)):0; const pay=Number(amount)||0; const after=Math.max(0,due-pay); const over=Math.max(0,pay-due);
  const save=()=>{if(!shopId||pay<=0)return alert("Select a shop and enter a payment amount.");const p:Payment={id:crypto.randomUUID(),shopId,date,amount:pay,method,notes,createdAt:new Date().toISOString()};setData(d=>({...d,payments:[...d.payments,p]}));setAmount("");setNotes("");alert("Payment recorded successfully.")};
  return <div className="two-col"><div className="panel form-panel"><h2>Record evening collection</h2><p className="muted">The amount due includes previous unpaid balance plus the selected date's supply.</p><div className="form-grid"><label>Date<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><label>Shop<select value={shopId} onChange={e=>setShopId(e.target.value)}><option value="">Select shop</option>{data.shops.filter(s=>s.active).map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label>Amount Collected *<input type="number" min="0" step="0.01" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="0"/></label><label>Payment Method<select value={method} onChange={e=>setMethod(e.target.value)}><option>Cash</option><option>UPI</option><option>Bank Transfer</option><option>Other</option></select></label><label className="wide">Notes<textarea value={notes} onChange={e=>setNotes(e.target.value)}/></label></div><button className="primary" onClick={save}><Banknote size={18}/> Save Collection</button></div>
  <div className="panel"><h2>Payment summary</h2><div className="balance-card"><div><span>Total due</span><b>{money(due)}</b></div><div><span>Collected now</span><b>{money(pay)}</b></div><div><span>Remaining balance</span><b>{money(after)}</b></div>{over>0&&<div className="credit"><span>Advance / credit</span><b>{money(over)}</b></div>}</div><div className="tip"><b>How it works</b><p>Previous balance + supply − payment = closing balance. The closing balance automatically carries to the next day.</p></div></div></div>
}
